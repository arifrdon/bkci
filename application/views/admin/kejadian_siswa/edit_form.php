...
<body id="page-top">
	...
	<div id="wrapper">
			...
			<div class="container-fluid">
				...
				<div class="card mb-3">
					...
					<div class="card-body">
						...
						<form action="<?php base_url('admin/kejadian_siswa/edit') ?>" method="post" enctype="multipart/form-data" >

                        <input type="hidden" name="id_kejadian_siswa" value="<?php echo $kejadiansw->ID_KEJADIAN_SISWA?>" />
							...
							<div class="form-group">
                                ...
                                <select placeholder="Pilih Siswa" class="form-control <?php echo form_error('no_induk') ? 'is-invalid':'' ?>" name="no_induk" id="no_induk" >
                                    <?php foreach($siswa as $sw){ ?>
                                        <option <?php if($kejadiansw->NO_INDUK == $sw->no_induk){echo "selected";} ?> value = "<?php echo $sw->no_induk; ?>"><?php echo $sw->nama_siswa; ?></option>
                                    <?php } ?>
								</select>
							...
							<div class="form-group">
                                ...
                                <select placeholder="Pilih Kejadian" class="form-control <?php echo form_error('id_daftar_kejadian') ? 'is-invalid':'' ?>" name="id_daftar_kejadian" id="id_daftar_kejadian" >
                                    <?php foreach($daftar_kejadian as $dk){ ?>
                                        ...
                                        <option <?php if ($kejadiansw->ID_DAFTAR_KEJADIAN == $dk->ID_DAFTAR_KEJADIAN){echo "selected";} ?> value = "<?php echo $dk->ID_DAFTAR_KEJADIAN; ?>"><?php echo $dk->NAMA_KEJADIAN; ?></option>
                                    <?php } ?>
                                </select>
                                <div class="invalid-feedback">
							...
							<div class="form-group">
                                <?php 
                                    $tanggaljadi = strtotime($kejadiansw->TANGGAL_KEJADIAN);
                                ?>
                                ...
                                <input type="text" id="single-input" value="<?php echo date('H:i', $tanggaljadi); ?>" name="jam" placeholder="Waktu" class="form-control <?php echo form_error('jam') ? 'is-invalid':'' ?>">
								<div class="invalid-feedback">
		...
		$(document).ready(function(){
			...
			$('#datepicker').datepicker("setDate", new Date(<?php echo date('Y', $tanggaljadi); ?>,<?php echo date('m', $tanggaljadi); ?>-1,<?php echo date('d', $tanggaljadi); ?>) );
